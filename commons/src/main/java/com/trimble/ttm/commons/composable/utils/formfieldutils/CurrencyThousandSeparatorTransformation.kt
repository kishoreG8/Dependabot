...
class CurrencyThousandSeparatorTransformation(private val formField: FormField) : VisualTransformation {
    override fun filter(text: AnnotatedString): TransformedText {

        val outputText = convertToCurrency(convertToThousandSeparatedString(value = text.text))
        ...
        val offsetMapping = object : OffsetMapping {
            ...
            override fun originalToTransformed(offset: Int): Int {
                val transformedOffset = offset + abs(text.length - outputText.length)
                return try {
                    transformedOffset
